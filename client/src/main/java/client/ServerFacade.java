...
public class ServerFacade {
    ...
    public int createGame(String token, String name) throws ServerError {
        ...
        if (response.statusCode() == 200) {
            var info_body =  new Gson().fromJson(response.body(), HashMap.class);
            double id = (double) info_body.get("gameID");
            return (int) id;
        }
        var error_body = new Gson().fromJson(response.body(), HashMap.class);
        String error_message = error_body.get("message").toString();
